"""
Unit tests for the _preview_name helper in wrangler_routes_sql.py.
...
"""
import hashlib
from app.wrangler_routes_sql import _preview_name
...
def test_exactly_56_chars_unchanged():
    ...
    result = _preview_name(base, suffix)
...
def test_errors_sibling_fits_for_short_name():
    result = _preview_name("sales", "_preview_delete")
...
def test_same_inputs_produce_same_output():
    ...
    assert _preview_name(base, suffix) == _preview_name(base, suffix)


def test_different_bases_produce_different_names():
    suffix = "_preview_delete"
    result_a = _preview_name("a" * 60, suffix)
    result_b = _preview_name("b" * 60, suffix)
